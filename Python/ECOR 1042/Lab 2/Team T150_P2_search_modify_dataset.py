...
def print_dictionary_category(category: str, booklist: dict):
    ...
    for items in booklist:
        if (items["genre"] == category):
            counter = counter + 1                
            lines.append({'text': f"{counter} - {items['title']}, was written by {items['author']}, has the rating {items['rating']}, was published by {items['publisher']}, has the page count of {items['pagecount']},  and is in {items['language']}", 'category': items['genre']})

    print('The category ' + category + ' has ' + str(counter) + ' books.  This is the list  of books in the category ' + category + ' :  ')  
    for line in lines:
        print(line['text'])    
    if (lines.__len__() == 0):
...
def get_books_by_rate(rating, bookList:list):
    ...
    for items in bookList:
        if (items["rating"] <= str(rating + 1)):
            counter = counter + 1
            print(str(counter) +'-'+ "title:", items["title"], "-""Authors:", items["author"], "-""Rating:", items["rating"],"-" "Publisher:", items["publisher"],"-""Category:", items["genre"], "-""Page Count:", items["pagecount"])
            books.append(items)
    if (books.__len__() == 0):
